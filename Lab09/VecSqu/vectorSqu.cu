 ...
 */
__global__
void vectorSqu(const float *A, float *C, unsigned long numElements)
{
...
int main(int argc, char** argv)
{
    ...
    vectorSqu<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_C, numElements);
    err = cudaGetLastError();
    checkErr(err, "Failed to launch vectorSqu kernel");
